...
const MeditComment: React.FC<ModalProps> = ({ isOpen, onCloseAction, comentarioId, conteudoAtual }) => {
  ...
  // Popula editor com conteúdo atual ao abrir modal
  useEffect(() => {
    if (isOpen && editorRef.current) {
      editorRef.current.innerHTML = conteudoAtual || "";
    }
  }, [isOpen, conteudoAtual]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Editando comentário:", { conteudo: conteudoRaw, userId, comentarioId });
      await updateComentario(comentarioId, conteudoRaw);
